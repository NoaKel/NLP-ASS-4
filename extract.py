...
class Data:
    def __init__(self, trainFile, trainAnnotations):
        ...
        print "Creating train data", passedTime()
...
def main((trainCorpusFile, devCorpusFile, trainAnnotationsFile, devAnnotationsFile)):
    if trainCorpusFile[:1] is '#' or devCorpusFile[:1] is '#':
    ...
    devData = Data(devCorpusFile, devAnnotationsFile)

    predicted = model.predict(devData.data)

    _, validationTags = model.extractFeatures(devData.data, False)

    print "Positive predictions", sum([0 if p == "None" else 1 for p in predicted])

    # I tried manually filtering the entity types here, and uppercase first letter, but it seems like the model
    # learned those pretty well and did not need my manual help

    falsePositives = 0
    falseNegatives = 0

    for i, (p, v) in enumerate(zip(predicted, validationTags)):
        if p != v:
            ((arg1, arg2, sentence), tag) = devData.data[i]
            print "----"
            print p, v, "/", arg1["text"], "/", arg2["text"]
    ...
    print predicted
    print "-----------------------"
    print metrics.classification_report(validationTags, predicted)

    devData.output_to_file('DEV.annotations.Pred', predicted)
    trainData.output_to_file('TRAIN.annotations.Pred', model.predict(trainData.data))
